...
RES_DIR = set_res_dir()
if TRAIN:
    command = ['python', 'train.py', '--data', '../data.yaml', '--weights', 'yolov5s.pt',
               '--img', '640', '--epochs', str(
                   EPOCHS), '--batch-size', '4', '--name', RES_DIR,
               '--device', '0']
...
def inference(RES_DIR, data_path):
    ...
    print(
        f"Current number of inference detection directories: {infer_dir_count}")
    INFER_DIR = f"inference_{infer_dir_count+1}"
    print(INFER_DIR)
    # Inference on images.
    subprocess.run(['python', 'detect.py', '--weights',
                   f'runs/train/{RES_DIR}/weights/best.pt', '--source', data_path, '--name', INFER_DIR])
    return INFER_DIR
...
download_file(
    'https://learnopencv.s3.us-west-2.amazonaws.com/yolov5_inference_data.zip', 'inference_data.zip')
